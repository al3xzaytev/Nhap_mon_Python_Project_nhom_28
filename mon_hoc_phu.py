...
def del_mon_hoc_phu():
    ...
    for entry in file:
        ...
        if mshv_entry[0] == mshv_input:
            ...
            continue
    if found:
        if count_mon_hoc_phu(mshv_input):
            while True:
...
def ma_mhp():
    print("\n[========== XEM ĐIỂM MÔN HỌC PHỤ THEO MÃ MÔN HỌC ==========]")
    while True:
        ...
        if mhp_so.isdigit():
            ...
            list_mhp = []
            for entry in danh_sach:
                if len(entry) >= index_dmhp:
                    list_mhp.append(entry[index_mmhp])
            list_mhp = set(list_mhp)

            if not list_mhp:
                print(f"\nThông báo: Danh sách không có học sinh nào có môn học phụ thứ {mhp_so}.")
            else:
                print(f"\nDanh sách môn học phụ {mhp_so}: ", end="")
                for entry in list_mhp:
                    last_entry = list(enumerate(list_mhp))[-1][-1]
                    if entry == last_entry:
                        print(entry)
                    else:
                        print(entry, end=", ")

                mhp_ma = input("Nhập mã môn học phụ cần xem điểm: ")
                if mhp_ma == "quit":
                    return
                else:
                    data = []
                    for entry in danh_sach:
                        if len(entry) >= index_dmhp and entry[index_mmhp] == mhp_ma:
                            ...
                            data.append(add)
                    if not data:
                        print(f"Thông báo: Danh sách không có môn học phụ số {mhp_so} theo mã {mhp_ma}.")
                        return
                    else:
...
def diem_mhp():
    print("\n[========== XEM ĐIỂM MÔN HỌC PHỤ THEO MÃ SỐ HỌC VIÊN ==========]")
    file = file_handling.process_file("read")
